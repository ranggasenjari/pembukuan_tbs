-- Sub Nota Komisi Pembawa
-- Mencatat margin yang diberikan ke supplier pembawa saat bon dibawa pihak ketiga.
-- Sub nota TIDAK ikut alur nota/pembayaran (status) — hanya sebagai catatan & laporan.
-- Total = netto_2 * price_per_kg (murni, tanpa potongan).

create table if not exists inv.sub_notas (
  id uuid primary key default gen_random_uuid(),
  bon_id uuid not null references inv.bons(id) on delete cascade,
  carrier_name text not null,
  price_per_kg integer not null,
  netto_2 integer not null,
...
create index if not exists sub_notas_bon_id_idx on inv.sub_notas (bon_id);

alter table inv.sub_notas enable row level security;

-- Akses untuk user terautentikasi. Pola disamakan dengan tabel operasional lain.
create policy "Authenticated users can manage sub_notas"
  on inv.sub_notas
  for all
  to authenticated
  using (true)
...
-- Keep updated_at segar (pola sama dengan bump_offline_sync_version).
create or replace function inv.bump_sub_nota_updated_at()
...
drop trigger if exists sub_notas_bump_updated_at on inv.sub_notas;
create trigger sub_notas_bump_updated_at
  before update on inv.sub_notas
  for each row execute function inv.bump_sub_nota_updated_at();

-- Dukungan offline: perluas apply_offline_sync untuk entity sub_nota / sub_nota_delete.
...
begin
  ...
  elsif p_entity_type = 'sub_nota' then
    v_id := (p_payload->'sub_nota'->>'id')::uuid;
    insert into inv.sub_notas (id, bon_id, carrier_name, price_per_kg, netto_2, amount, notes)
      values (
        v_id,
        (p_payload->'sub_nota'->>'bon_id')::uuid,
        p_payload->'sub_nota'->>'carrier_name',
        coalesce((p_payload->'sub_nota'->>'price_per_kg')::integer, 0),
        coalesce((p_payload->'sub_nota'->>'netto_2')::integer, 0),
        coalesce((p_payload->'sub_nota'->>'amount')::integer, 0),
        nullif(p_payload->'sub_nota'->>'notes', '')
      )
      on conflict (id) do update set
        bon_id = excluded.bon_id,
        carrier_name = excluded.carrier_name,
        price_per_kg = excluded.price_per_kg,
        netto_2 = excluded.netto_2,
        amount = excluded.amount,
        notes = excluded.notes
      returning sync_version into v_version;
    v_result := jsonb_build_object('status','ok','sync_version',v_version);

  elsif p_entity_type = 'sub_nota_delete' then
    v_id := (p_payload->>'sub_nota_id')::uuid;
    delete from inv.sub_notas where id = v_id;
    if found then v_result := jsonb_build_object('status','ok');
    else v_result := jsonb_build_object('status','ok','deleted',false); end if;
