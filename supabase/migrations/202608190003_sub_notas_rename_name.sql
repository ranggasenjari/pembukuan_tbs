-- Rename kolom carrier_name -> name (sub nota adalah entitas umum, bukan hanya komisi pembawa).
-- Data yang ada dipertahankan oleh ALTER ... RENAME.
alter table inv.sub_notas rename column carrier_name to name;

-- Sinkronisasi offline: update branch sub_nota agar memakai kolom name.
...
begin
  ...
  if p_entity_type = 'bon' then
    ...
    insert into inv.sub_notas (id, bon_id, name, price_per_kg, netto_2, amount, notes)
      values (
        v_id,
        (p_payload->'sub_nota'->>'bon_id')::uuid,
        p_payload->'sub_nota'->>'name',
